...
def string_to_float():
    def robust_string_to_float(x):
        try:
            if type(x) == str: return float(x)
            else: return x
        except:
            return x
    return robust_string_to_float
...
def ngram_tf(n, min_doc_freq, max_doc_freq, variables, print_stats=False):
    """
    Get ngram dictionary from pandas series
    :param n: int
...
def ngram_idf_sum(variable, ngram_tf_df, n):
    ...
    :param variable: pandas.Series
    :param ngram_tf_df: pandas.DataFrame
    :param n: int
    :return: pandas.Series
    """
    return variable.apply(lambda x: ngram_idf_sum_on_string(x, ngram_tf_df, n))
...
def ngram_idf_sum_on_string(string, ngram_tf_df, n):
    """
